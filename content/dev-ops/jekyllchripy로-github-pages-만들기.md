...
date = "2022-05-01T01:11:00.000+09:00"
...
notion_url = "https://app.notion.com/p/jekyll-Chripy-GitHub-Pages-3a0aac4e32a58183aee8d3620933cfe3"
+++

# Chirpy 가져오기

1. Chirpy 저장소의 [Release](https://github.com/cotes2020/jekyll-theme-chirpy/releases)에서 소스를 다운로드하거나
1. Chirpy 저장소를 [Fork](https://github.com/cotes2020/jekyll-theme-chirpy/fork) 합니다.
...
1. Latest Release 버전의 소스 파일 `Source code (zip)`을 로컬 환경에 다운로드 후 압축을 해제합니다.
1. 터미널로 해당 폴더에 접속합니다.
1. `.gitignore` 파일에 제외할 파일을 추가해줍니다.
   ```bash
echo "Gemfile.lock" >> .gitignore
   ```
1. 해당 폴더를 git 저장소로 생성합니다.
   ```bash
git init
   ```
1. 생성한 GitHub 저장소로 원격 연결합니다.
   ```bash
git remote add origin https://github.com/<GitHub 계정명>/<GitHub 계정명>.github.io.git
   ```
1. `main` branch를 생성하고 압축 해제된 파일들을 commit합니다.
   ```bash
git branch -M main
git add .
...
1. 방금 Fork하여 생성한 저장소를 로컬에 Clone 합니다.
   ```bash
...
1. 설치된 프로그램 중 `Start Command Prompt with Ruby`를 실행합니다.
1. gem 명령어를 통해 jekyll과 패키지들을 설치합니다.
   ```bash
gem install jekyll
gem install minima
gem install bundler
